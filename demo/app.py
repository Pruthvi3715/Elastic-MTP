...
def load_model(model_name: str):
    ...
    try:
        ...
        print(f"Notice: Loading fallback 'Qwen/Qwen2.5-0.5B-Instruct' ({e})...")
        fallback_name = "Qwen/Qwen2.5-0.5B-Instruct"
        tokenizer = AutoTokenizer.from_pretrained(fallback_name, trust_remote_code=True)
        model = AutoModelForCausalLM.from_pretrained(fallback_name, torch_dtype=torch.float32, trust_remote_code=True).to(device)
        model.eval()
