...
def generate_all_improved_charts():
    ...
    milestones = ["M1: Base", "M2: Static", "M3: 1D Elastic", "M4: Post-Trained", "M5: 2D Tree", "M6: vLLM CUDA"]
    ...
    concurrency = [16, 16, 64, 64, 48, 256]
    router_lat = [0.000, 0.000, 0.480, 0.480, 0.450, 0.018]
    costs = [10.00, 8.50, 4.20, 3.10, 2.40, 0.17]
    ...
    for b in b1:
        ax1.text(b.get_x() + b.get_width()/2, b.get_height() + 0.15, f"{b.get_height():.2f}x", ha="center", fontweight="bold", fontsize=9)
    ...
    for b in b2:
        if b.get_height() > 0:
            ax2.text(b.get_x() + b.get_width()/2, b.get_height() + 2.5, f"{b.get_height():.1f}%", ha="center", fontweight="bold", fontsize=9)
    ...
    for b in b3:
        ax3.text(b.get_x() + b.get_width()/2, b.get_height() + 5.0, f"{b.get_height():.1f}", ha="center", fontweight="bold", fontsize=9)
    ...
    for b in b4:
        ax4.text(b.get_x() + b.get_width()/2, b.get_height() + 6.0, f"{int(b.get_height())}", ha="center", fontweight="bold", fontsize=9)
    ...
    for b in b5:
        if b.get_height() > 0:
            ax5.text(b.get_x() + b.get_width()/2, b.get_height() + 0.015, f"{b.get_height():.3f}ms", ha="center", fontweight="bold", fontsize=9)
