...
class GatedLoRAPredictionHead(nn.Module):
    ...
    def forward(self, z_t: torch.Tensor, prev_token_emb: Optional[torch.Tensor] = None) -> torch.Tensor:
...
@register_adapter("mtp_glora")
class MTPGLoRAModule(nn.Module, BaseAdapter):
    def __init__(self, 
                 hidden_dim: int = 1536, 
                 vocab_size: int = 151936, 
                 num_aux_heads: int = 3, 
    ...
    def forward(self, hidden_states: torch.Tensor) -> List[torch.Tensor]:
        return [head(hidden_states) for head in self.aux_heads]
